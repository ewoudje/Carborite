...
public class PacketDataSerializer extends ByteBuf {
    ...
    public PacketDataSerializer writeBlockPosition(BlockPosition blockposition) {
        this.writeLong(blockposition.asLong());
        return this;
    }

    public Object readIChatBaseComponent() { //IChatBaseComponent
        return null;
    }

    public PacketDataSerializer writeIChatBaseComponent() {
        return this;
    }
    ...
    /*
    public PacketDataSerializer difficulty(@Nullable NBTTagCompound nbttagcompound) {
        if (nbttagcompound == null) {
            this.writeByte(0);
        } else {
            try {
                NBTCompressedStreamTools.difficulty(nbttagcompound, (DataOutput) (new ByteBufOutputStream(this)));
    ...
    public NBTTagCompound j() {
        int asFloat = this.readerIndex();
        byte b0 = this.readByte();

        if (b0 == 0) {
            return null;
        } else {
            this.readerIndex(asFloat);

            try {
                return NBTCompressedStreamTools.difficulty((DataInput) (new ByteBufInputStream(this)), new NBTReadLimiter(2097152L));
            } catch (IOException ioexception) {
                throw new EncoderException(ioexception);
            }
        }
    ...
    public PacketDataSerializer difficulty(ItemStack itemstack) {
        if (itemstack.isEmpty()) {
            ...
            this.difficulty(nbttagcompound);
        }
...
/*
    ...
    public PacketDataSerializer difficulty(MinecraftKey minecraftkey) {
        this.difficulty(minecraftkey.toString());
        return this;
    }
    */
